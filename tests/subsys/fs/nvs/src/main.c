...
static size_t num_occupied_cache_entries(const struct nvs_fs *fs)
{
...
ZTEST_F(nvs, test_nvs_cache_hash_quality)
{
	...
	/* Write CONFIG_NVS_LOOKUP_CACHE entries following the pattern used by Settings over NVS */

	for (i = 0; i < CONFIG_NVS_LOOKUP_CACHE_SIZE / 2; i++) {
		uint16_t id = NVS_NAMECNT_ID + 1 + (uint16_t)i;

		err = nvs_write(&fixture->fs, id, key, sizeof(key) - 1);
		zassert_equal(err, sizeof(key) - 1, "nvs_write call failure: %d", err);

		err = nvs_write(&fixture->fs, id + NVS_NAME_ID_OFFSET, &data, sizeof(data));
		zassert_equal(err, sizeof(data), "nvs_write call failure: %d", err);
	}

	/* Verify that at least 60% cache entries are occupied */

	const size_t MIN_CACHE_OCCUPANCY_LEVEL = CONFIG_NVS_LOOKUP_CACHE_SIZE * 6 / 10;

	num = num_occupied_cache_entries(&fixture->fs);
	TC_PRINT("Cache occupancy: %u/%u\n", (unsigned)num, (unsigned)CONFIG_NVS_LOOKUP_CACHE_SIZE);
	zassert_between_inclusive(num, MIN_CACHE_OCCUPANCY_LEVEL, CONFIG_NVS_LOOKUP_CACHE_SIZE,
				  "too low cache occupancy - it may indicate poor hash quality");
#endif
}
